    ...
    if(isset($_POST['submit']))
  {
   ...
   $remark=$_POST['remark'];
    $tcost=$_POST['cost'];
      ...
      $query.=mysqli_query($con, "update   bookingmoto set TotalCost='$tcost', Status='$status' ,Remark='$remark' where ID='$vid'");
    if ($query) {
    echo '<script>alert("Transporta rezervacijas bija atjaunoti.")</script>';
    echo "<script>window.location.href ='all-twowheeler-booking.php'</script>";
  }
  else
    {
      echo '<script>alert("Kluda. Meginiet vel reiz")</script>';
    }
...
<head>
    <title>Admin panele | Moto rezervacijas detalas</title>
       ...
       <?php include_once('includes/header.php');?>
            ...
            <section class="tables-section">
                ...
                <div class="outer-w3-agile mt-3">
                    <h4 class="tittle-w3-agileits mb-4">Moto rezervacija</h4>
                    <div class="container-fluid">
                        <div class="row">
                            
                                <?php
                               $vid=$_GET['viewid'];
$ret=mysqli_query($con,"select bookingmoto.ID as bid,DATEDIFF(bookingmoto.ReturnDate,bookingmoto.BookingDate) as ddf,bookingmoto.FullName,bookingmoto.Email,bookingmoto.MobileNumber,bookingmoto.Location,bookingmoto.BookingNumber,bookingmoto.BookingDate,bookingmoto.ReturnDate,bookingmoto.Status,moto.BrandName,moto.VehicleName,moto.RegistrationNumber,moto.RentalPrice,moto.VehicleModel,moto.VehicleDescription,bookingmoto.CreationDate from  bookingmoto join  moto on moto.ID=bookingmoto.VehicleID where bookingmoto.ID='$vid'");
$cnt=1;
...
<td colspan="4" style="font-size:20px;color:blue">
 Lietotaja detalas</td></tr>
  ...
  <tr>
    <th>Trasnporta registracijas nr.</th>
...
<?php  if($status!=''){
$ret=mysqli_query($con,"select * from bookingmoto  where bookingmoto.ID='$vid'");
$cnt=1;
...
<table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
  <tr align="center">
   <th colspan="5" >Rezervacijas vesture</th> 
  </tr>
  <tr>
    <th>Nr.</th>
<th>Kopiga summa</th>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <tr>
   ...
   <select name="status" class="form-control wd-450" required="true" >
     ...
     <option value="Unapproved">Atcelt</option>
   </select></td>
  </tr>
</table>
</div>
